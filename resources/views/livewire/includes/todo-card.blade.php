<div wire:key={{ $todo->id }} class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500
    ...
    <div class="flex justify-between space-x-2">
        <div class="flex">
            <input type="checkbox" id="" class="mr-2" wire:model="completed" {{ $todo->completed ? 'checked' : '' }}
            wire:click="toggle({{ $todo->id }})">
            @if ($editMode && $todo->id == $editTodoId)
            <input type="text" placeholder="Todo.."
                class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5" id="editTodoName"
                wire:model="editTodoName">
